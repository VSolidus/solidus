...
*Setup instructions for a Gitian build of VSolidus Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Solidus
...
- [Building VSolidus Core](#building-solidus-core)
...
Clone the git repositories for solidus and Gitian.
...
in the solidus repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building VSolidus Core
----------------

To build VSolidus Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the solidus repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/solidus/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/solidus-project/solidus
...
For example:
```bash
URL=https://github.com/thrasher-/solidus.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit solidus=${COMMIT} --url solidus=${URL} ../solidus/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit solidus=${COMMIT} --url solidus=${URL} ../solidus/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit solidus=${COMMIT} --url solidus=${URL} ../solidus/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the solidus git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../solidus/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/solidus
SIGPATH=/some/root/path/solidus-detached-sigs

./bin/gbuild --url solidus=${BTCPATH},signature=${SIGPATH} ../solidus/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solidus-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solidus-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/solidus-osx-build.assert
```
...
[solidus-project/gitian.sigs.sol](https://github.com/solidus-project/gitian.sigs.sol/) repository, or if that's not possible create a pull
