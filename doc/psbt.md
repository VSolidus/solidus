# PSBT Howto for Solidus Core

Since Solidus Core 0.17, an RPC interface exists for Partially Signed Solidus
...
PSBT is an interchange format for Solidus transactions that are not fully signed
...
#### Multisig with multiple Solidus Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Solidus Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `solidus-cli` is used.
