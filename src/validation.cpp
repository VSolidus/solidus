                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s SOL additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if (nHeight == 1)
		nSubsidy = 4611686018 * COIN;
	else if (nHeight == 3)
		nSubsidy = 4611686018 * COIN;
	else if (nHeight < 814680)
		nSubsidy = 300 * COIN;
	else
		nSubsidy = 150 * COIN;
...
{
	return false; //Needs to always return false as SEGWIT won't work from block 1 otherwise.
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.BIP34Height)
    {
        ...
        if (block.vtx[0]->vin[0].scriptSig.size() < expect.size() ||
            ...
            //return state.DoS(100, false, REJECT_INVALID, "bad-cb-height", false, "block height mismatch in coinbase"); //Needs to always return true as SEGWIT won't work from block 1 otherwise.
        }
    }
